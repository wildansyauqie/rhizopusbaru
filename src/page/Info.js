...
import { IoIosWater } from "react-icons/io";
import Moment from "react-moment";
import { useSubscription, useFilter } from "react-supabase";
...
export default function Info() {
  ...
  const [{ data, error }] = useSubscription("maintable", {
    select: { filter },
  });
  ...
  const api = data[data.length - 1];
  const fault = now - new Date(api.timestamp) > 300000 ? true : false;

  return (
    <div className="flex flex-col w-full justify-center items-center" style={{ height: "calc(100% - 6rem)" }}>
      <div>
        <div className="text-stone-700 dark:text-stone-300">
          <div className="flex flex-row gap-2 justify-center items-center mb-4">
            <div className={"w-2 h-2 rounded-full animate-pulse " + (fault ? "bg-red-600" : "bg-green-600")} />
            <div className="text-xs flex flex-row gap-1 items-center">
              {fault ? (
                <p className="opacity-80">Cek perangkat anda!</p>
              ) : (
              ...
              {fault && (
                <div className="dropdown dropdown-end">
                  ...
                  <div className="modal modal-bottom sm:modal-middle bg-black bg-opacity-60">
                    <div className="modal-box bg-gray-100 dark:bg-slate-800">
                      <h3 className="font-bold text-lg">Perangkat tidak mengirim data selama 5 menit!</h3>
                      <p className="py-4">
                        Ini dapat disebabkan oleh perangkat yang tidak tersmbung dengan daya, koneksi internet ataupun terjadi kesalahan pada sensor.
                      </p>
                      <div className="modal-action">
                        <label htmlFor="my-modal-6" className="btn btn-primary">Mengerti</label>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="stats shadow-md bg-gray-200 dark:bg-slate-800 stats-vertical lg:stats-horizontal mb-2">
            {['Suhu1', 'Suhu2', 'Kelembapan1', 'Kelembapan2', 'Berat1', 'Berat2'].map((type) => (
              <div className="stat" key={type}>
                ...
                <div className="stat-value text-cyan-900 dark:text-stone-300">
                  {isString(api[type]) ? api[type] : api[type].toFixed(1)}
                </div>
              </div>
            ))}
          </div>
          <p className="text-xs opacity-60 text-center">
            Diperbaharui <Moment format="DD MMMM YYYY, HH:mm">{api.timestamp}</Moment>
          </p>
        </div>
      </div>
    </div>
